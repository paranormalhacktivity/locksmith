require 'rspec'
require_relative '../lib/polyalphabetic_substitution_cipher'

RSpec.describe 'Polyalphabetic Substitution Cipher' do
  let(:cipher) { PolyalphabeticSubstitutionCipher.new(key: 'SECRET') }
  ...
  describe 'encrypt' do
    it 'encrypts a string using the Polyalphabetic Substitution cipher' do
    ...
    it 'handles repeating short keys for long text' do
      cipher = PolyalphabeticSubstitutionCipher.new(key: 'A')  # A simple key that shouldn't change the text
      result = cipher.encrypt('LONGTEXTWITHSHORTKEY')
      expect(result).to eq('LONGTEXTWITHSHORTKEY')  # A key of 'A' leaves the text unchanged

      cipher = PolyalphabeticSubstitutionCipher.new(key: 'B')  # A key that will actually cause a shift, just to feel good
    ...
    it 'treats the key as case-insensitive' do
      cipher_with_uppercase_key = PolyalphabeticSubstitutionCipher.new(key: 'SECRET')
      cipher_with_lowercase_key = PolyalphabeticSubstitutionCipher.new(key: 'secret')
  ...
  describe 'error_handling' do
    it 'raises an error when the key is empty' do
      expect { PolyalphabeticSubstitutionCipher.new(key: '') }.to raise_error(ArgumentError, "Key cannot be empty")
    end

    it 'raises an error when the key contains non-alphabetic characters' do
      expect { PolyalphabeticSubstitutionCipher.new(key: 'ABC1!') }.to raise_error(ArgumentError, "Key must contain only alphabetic characters")
    end
  end
end
